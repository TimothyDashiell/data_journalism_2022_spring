...
**Answer** State name, voting resaults from in 2016, and PPP loan info
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Not a strong a liner relationship as the last graph
```{r}
...
**Answer** The value for Businesses recieving PPE have a very high percentage of Trump votes
```{r}
reveal_data %>%
...
**Answer** The code runs a T Test. The data supports the second/alternative hypothesis
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The code runs another T Test, this time the resaults show that the null hypothis is supported
```{r}
...
**Answer** AV=verage ppp amount per employee for Clinton and Trump states, and mean business receiving PPP loanss for Clinton and Trump  states
...
**Answer** After the T test you see two means, one for Trump and another for Clinton. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The data supports the hypothesis because of the true diffrent in means on both the Trump and Clinton sides
```{r}
