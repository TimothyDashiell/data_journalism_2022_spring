...
```{r}
ppp_maryland_loans <- read_csv("ppp_loans_md.csv", guess_max=10)
...
```{r}
ppp_maryland_loans <- read_csv("ppp_loans_md.csv", guess_max=195865)
```
...
**Answer**: There are 192,959	loans where the franchise number is NA and 275 where the franchise name is Subway.
...
**Answer**:135,073 which means I can conclude that the amount of forgiven loans will be the same, we just (for now) dont know much money was forgiven
...
**Answer** The largest loan is for 10,000000 and the smallest is 6. The huge gap between the two makes it kinda suspcipus and makes we want to look into the companies these loans were for just a tad bit more. 
```{r}
...
**Answer** All of the fields align with the loan amount of that specfic business 
...
**Answer** The column named need to be cleaned, the data type needs to be changed or there are duplicates in the data. 
...
**Answer** Fixed Street Address and added _, made the "name" lowercase
...
**Answer** It says there is a problem with Problem with summarize coloumn "total amount" and that there is an invalid "type". I think it means that part of the data needs to be cleaned in order to properly summarize the data into 
...
**Answer** 2 rows, Stuart Jones LLC
...
**Answer** The code removed all the extra numbers so that every zip code was the same number of digits.
