...
**A4** Zip Code- 20850, Percentage-23.5% Count of loans- 240
...
**A5** It could be because a lot of Asian's didn't report themselves as Asian on the application. (Which could be for various reasons)
...
**A6** Between the 3 zip codes, there were a total of 13 loans for a total of $7,253,407.50

```{r}
maryland_ppp_with_naics_and_demographics %>% 
filter (race == "Asian",PNHA=="0",NHA=="0") %>% 
group_by(zip5,PNHA,NHA) %>% 
summarize(
    count = n(),
    total = sum (amount)) %>% 
  arrange(desc(total)) 
...
**A7** 44, to me the most news worthy one would have to be the Ammunition Manufacturing because it's one of the highest totals here and it does'nt include small arms because they have a category of their own. So I would do futher research of this ammunition company that needs SO much loan money. Is it because they have some sort of contract with a huge buyer? 
```{r}
maryland_ppp_with_naics_and_demographics %>% 
group_by(title) %>%
summarize(
