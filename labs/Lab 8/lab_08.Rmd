...
```{r}
PPPDATA<- read_rds("data/lab_08.rds")
```
...
```{r}
spanish5to17<- get_acs(geography = "county",
              variables = c(medspeaker = "B16007_004"),
              state = "MD",
              year = 2019)

total5to17<- get_acs(geography = "county",
              variables = c(medspeaker = "B16007_002"),
              state = "MD",
              year = 2019)

total5to17 %>% inner_join(spanish5to17,by=c("GEOID","NAME")) %>%   
rename(TOTAL=estimate.x, Spanish=estimate.y) %>% 
...
**A4** I think that what stands out about the top 5 is PG and MoCo, i think it makes sense that such diverse areas are inside the top 5. I fully expected Garrett county to be at the bottom just because Garrett county is less diverse and honestly full of white people.

**Q5** Which 5-digit zip code had the most removed PPP applications, and what is the percentage of Black population in that zip code according to the ACS? To answer this question, you will need to 

create separate dataframes with ACS data on total population by zip code and 

Black population by zip code. 
...
```{r}
Blackpop <-get_acs(geography = "zcta",
              variables = c(medspeaker = "B02001_003"),
              year = 2019)

USpop <-get_acs(geography = "zcta",
              variables = c(medspeaker = "B02001_001"),
              year = 2019)

combined<-
USpop %>% inner_join(Blackpop,by=c("GEOID","NAME")) %>% 
rename(TOTAL=estimate.x, BlackPeople=estimate.y) %>% 
mutate(Percentage=BlackPeople/TOTAL*100) %>%  
